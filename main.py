...
def update_system_prompt(current_iteration: Optional[int] = None,
...
async def generate_edit_instructions(file_path, file_content, instructions,
                                     project_context, full_file_contents):
    ...
    try:
        ...
        edit_instructions = parse_search_replace_blocks(
        ...
        console.print(f"Error in generating edit instructions: {str(e)}",
                      style="bold red")
...
def parse_search_replace_blocks(response_text, use_fuzzy=USE_FUZZY_SEARCH):
    ...
    for search, replace in matches:
        ...
        if use_fuzzy and search not in response_text:
            ...
            best_match = difflib.get_close_matches(search, [response_text],
                                                   n=1,
                                                   cutoff=0.6)
            if best_match:
                similarity = difflib.SequenceMatcher(None, search,
                                                     best_match[0]).ratio()
            else:
                similarity = 0.0
    ...
    for file in files:
        ...
        try:
            ...
            edit_instructions = await generate_edit_instructions(
                path, original_content, instructions, project_context,
                file_contents)
            ...
            if edit_instructions:
                ...
                edited_content, changes_made, failed_edits, console_output = await apply_edits(
                ...
                if changes_made:
                    ...
                    if failed_edits:
                        results.append({
                            ...
                            "message":
                            f"Some changes applied to {path}, but some edits failed.",
                        ...
                        results.append({
                            ...
                            "status": "success",
                            "message":
                            f"All changes successfully applied to {path}",
                            "edited_content": edited_content
                        })
                else:
                    results.append({
                        ...
                        "no_changes",
                        "message":
                        f"No changes could be applied to {path}. Please review the edit instructions and try again."
                    })
            else:
                results.append({
                    "path":
                    ...
                    "no_instructions",
                    "message":
                    f"No edit instructions generated for {path}"
                })
        except Exception as e:
...
async def apply_edits(file_path, edit_instructions, original_content):
    ...
    with Progress(SpinnerColumn(),
        ...
        edit_task = progress.add_task("[cyan]Applying edits...",
                                      total=total_edits)
        ...
        for i, edit in enumerate(edit_instructions, 1):
            ...
            if match or (USE_FUZZY_SEARCH and similarity >= 0.8):
                if not match:
                    ...
                    best_match = difflib.get_close_matches(search_content,
                                                           [edited_content],
                                                           n=1,
                                                           cutoff=0.6)
                    if best_match:
                        match = re.search(re.escape(best_match[0]),
                                          edited_content)
                ...
                if match:
                    ...
                    replace_content_cleaned = re.sub(r'</?SEARCH>|</?REPLACE>',
                                                     '', replace_content)
                    edited_content = edited_content[:
                                                    start] + replace_content_cleaned + edited_content[
                                                        end:]
                    changes_made = True

                    # Display the diff for this edit
                    diff_result = generate_diff(search_content,
                                                replace_content, file_path)
                    console.print(
                        Panel(
                            diff_result,
                            title=
                            f"Changes in {file_path} ({i}/{total_edits}) - Similarity: {similarity:.2f}",
                            style="cyan"))
...
tools = [{
    "name": "create_folders",
    "description":
    "Create new folders at the specified paths. This tool should be used when you need to create one or more directories in the project structure. It will create all necessary parent directories if they don't exist.",
    "input_schema": {
        "type": "object",
        "properties": {
            "paths": {
                ...
                "description":
                "An array of absolute or relative paths where the folders should be created. Use forward slashes (/) for path separation, even on Windows systems."
            }
        },
        "required": ["paths"]
    }
    ...
    "name": "scan_folder",
    "description":
    "Scan a specified folder and create a Markdown file with the contents of all coding text files, excluding binary files and common ignored folders.",
    "input_schema": {
        "type": "object",
        "properties": {
            "folder_path": {
                ...
                "string",
                "description":
                "The absolute or relative path of the folder to scan. Use forward slashes (/) for path separation, even on Windows systems."
            },
            "output_file": {
                "type":
                "string",
                "description":
                "The name of the output Markdown file to create with the scanned contents."
            }
        },
    ...
    "name": "create_files",
    "description":
    "Create one or more new files with the given contents. This tool should be used when you need to create files in the project structure. It will create all necessary parent directories if they don't exist.",
    "input_schema": {
        "type": "object",
        "properties": {
            "files": {
                ...
                "items": {
                    ...
                    "properties": {
                        "path": {
                            ...
                            "string",
                            "description":
                            "The absolute or relative path where the file should be created. Use forward slashes (/) for path separation, even on Windows systems."
                        },
                        "content": {
                            "type":
                            "string",
                            "description":
                            "The content of the file. This should include all necessary code, comments, and formatting."
                        }
                    },
    ...
    "description":
    "Apply AI-powered improvements to multiple files based on specific instructions and detailed project context.",
    "input_schema": {
        "type": "object",
        "properties": {
            "files": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "instructions": {
                            ...
                            "string",
                            "description":
                            "Specific instructions for editing this file."
                        }
                    },
            ...
            "project_context": {
                ...
                "string",
                "description":
                "Comprehensive context about the project, including recent changes, new variables or functions, interconnections between files, coding standards, and any other relevant information that might affect the edits."
            }
        },
    ...
    "input_schema": {
        ...
        "properties": {
            "code": {
                ...
                "string",
                "description":
                "The Python code to execute in the 'code_execution_env' virtual environment. Include all necessary imports and ensure the code is complete and self-contained."
            }
        },
        "required": ["code"]
    }
    ...
    "input_schema": {
        ...
        "properties": {
            "process_id": {
                ...
                "string",
                "description":
                "The ID of the process to stop, as returned by the execute_code tool for long-running processes."
            }
        },
        "required": ["process_id"]
    }
    ...
    "description":
    "Read the contents of one or more existing files at once. This tool now handles both single and multiple file reads. Use this when you need to examine or work with file contents.",
    "input_schema": {
        "type": "object",
        "properties": {
            "paths": {
                "oneOf": [{
                    ...
                    "string",
                    "description":
                    "The absolute or relative path of a single file to read."
                }, {
                    "type":
                    "array",
                    "items": {
                    ...
                    "description":
                    "An array of absolute or relative paths of the files to read."
                }],
                "description":
                "The path(s) of the file(s) to read. Use forward slashes (/) for path separation, even on Windows systems."
            }
        },
        "required": ["paths"]
    }
    ...
    "input_schema": {
        ...
        "properties": {
            "path": {
                ...
                "string",
                "description":
                "The absolute or relative path of the folder to list. Use forward slashes (/) for path separation, even on Windows systems. If not provided, the current working directory will be used."
            }
        }
    }
}, {
    ...
    "input_schema": {
        ...
        "properties": {
            "query": {
                ...
                "string",
                "description":
                "The search query. Be as specific and detailed as possible to get the most relevant results."
            }
        },
        "required": ["query"]
    }
...
async def decide_retry(tool_checker_response, edit_results):
    try:
        response = client.messages.create(
            ...
            max_tokens=1000,
            system=
            "You are an AI assistant tasked with deciding whether to retry editing files based on the previous edit results and the AI's response. Respond with a JSON object containing 'retry' (boolean) and 'files_to_retry' (list of file paths).",
            messages=[{
                "role":
                "user",
                "content":
                f"Previous edit results: {json.dumps(edit_results)}\n\nAI's response: {tool_checker_response}\n\nDecide whether to retry editing any files."
            }])
        ...
        console.print(
            Panel(f"Error parsing JSON in decide_retry: {str(e)}",
                  title="Error",
                  style="bold red"))
        return {"retry": False, "files_to_retry": []}
    except Exception as e:
        console.print(
            Panel(f"Error in decide_retry: {str(e)}",
                  title="Error",
                  style="bold red"))
        return {"retry": False, "files_to_retry": []}
...
async def execute_tool(tool_name: str,
    ...
    try:
        ...
        if tool_name == "create_files":
            ...
            process_id, execution_result = await execute_code(
                tool_input["code"])
            ...
            result = scan_folder(tool_input["folder_path"],
                                 tool_input["output_file"])
        else:
            is_error = True
        ...
        logging.error(
            f"Missing required parameter {str(e)} for tool {tool_name}")
        return {
            "content":
...
async def send_to_ai_for_executing(code, execution_result):
    ...
    try:
        ...
        console.print(f"Error in AI code execution analysis: {str(e)}",
                      style="bold red")
            ...
            if isinstance(message['content'], str):
                ...
                for content in message['content']:
                    if content['type'] == 'tool_use':
                        ...
                        formatted_chat += f"## Claude\n\n{content['text']}\n\n"
        elif message['role'] == 'user' and isinstance(message['content'],
                                                      list):
            for content in message['content']:
...
async def chat_with_claude(user_input,
    ...
    if image_path:
        console.print(
            Panel(f"Processing image at path: {image_path}",
                  title_align="left",
        ...
        if image_base64.startswith("Error"):
            console.print(
                Panel(f"Error encoding image: {image_base64}",
                      title="Error",
                      style="bold red"))
        ...
        console.print(
            Panel("Image message added to conversation history",
                  title_align="left",
                  title="Image Added",
                  style="green"))
    else:
        ...
        if isinstance(message['content'], list):
            filtered_content = [
                ...
                if content.get('type') != 'tool_result' or (
                    content.get('type') == 'tool_result' and not any(
                        keyword in content.get('output', '') for keyword in [
    ...
    try:
        ...
        response = client.beta.prompt_caching.messages.create(
            ...
            system=[{
                ...
                "text",
                "text":
                update_system_prompt(current_iteration, max_iterations),
                "cache_control": {
                    "type": "ephemeral"
                }
            }, {
        ...
        if e.status_code == 429:
            console.print(
                Panel("Rate limit exceeded. Retrying after a short delay...",
                      title="API Error",
                      style="bold yellow"))
            time.sleep(5)
            return await chat_with_claude(user_input, image_path,
                                          current_iteration, max_iterations)
        else:
            console.print(
                Panel(f"API Error: {str(e)}",
                      title="API Error",
                      style="bold red"))
    ...
    console.print(
        Panel(Markdown(assistant_response),
    ...
    for tool_use in tool_uses:
        ...
        console.print(
            Panel(f"Tool Input: {json.dumps(tool_input, indent=2)}",
                  style="green"))
        ...
        if isinstance(tool_result, dict) and tool_result.get("is_error"):
            console.print(
                Panel(tool_result["content"],
                      title="Tool Execution Error",
                      style="bold red"))
        else:
            # Format the tool result content for proper rendering
            formatted_result = json.dumps(tool_result, indent=2) if isinstance(
                tool_result, (dict, list)) else str(tool_result)
            console.print(
                Panel(formatted_result,
                      title_align="left",
                      title="Tool Result",
                      style="green"))
        ...
        tool_result_content = {
            ...
            "text",
            "text":
            json.dumps(tool_result) if isinstance(tool_result,
                                                  (dict,
                                                   list)) else str(tool_result)
        }
        ...
        current_conversation.append({
            ...
            "content": [{
                ...
                "is_error":
                tool_result.get("is_error", False) if isinstance(
                    tool_result, dict) else False
            }]
        })

        # Update the file_contents dictionary if applicable
        if tool_name in [
                'create_files', 'edit_and_apply_multiple',
                'read_multiple_files'
        ] and not (isinstance(tool_result, dict)
                   and tool_result.get("is_error")):
            if tool_name == 'create_files':
                for file in tool_input['files']:
                    if "File created and added to system prompt" in str(
                            tool_result):
                        file_contents[file['path']] = file['content']
            elif tool_name == 'edit_and_apply_multiple':
                edit_results = tool_result if isinstance(
                    tool_result, list) else [tool_result]
        ...
        try:
            ...
            console.print(
                Panel(Markdown(tool_checker_response),
            ...
            if tool_name == 'edit_and_apply_multiple':
                retry_decision = await decide_retry(tool_checker_response,
                                                    edit_results)
                if retry_decision["retry"]:
                    ...
                    console.print(
                        Panel(retry_console_output,
                              title="Retry Result",
                              style="cyan"))
                    assistant_response += f"\n\nRetry result: {json.dumps(retry_result, indent=2)}"
                else:
                    console.print(
                        Panel("Clude has decided not to retry editing",
                              style="green"))

        except APIError as e:
            error_message = f"Error in tool response: {str(e)}"
            console.print(Panel(error_message, title="Error",
                                style="bold red"))
            assistant_response += f"\n\n{error_message}"
...
def reset_code_editor_memory():
    ...
    code_editor_memory = []
    console.print(
        Panel("Code editor memory has been reset.",
              title="Reset",
              style="bold green"))
...
def display_token_usage():
    ...
    for model, tokens in [("Main Model", main_model_tokens),
        ...
        output_cost = (output_tokens /
                       1_000_000) * model_costs[model]["output"]
        cache_write_cost = (cache_write_tokens /
                            1_000_000) * model_costs[model]["cache_write"]
        cache_read_cost = (cache_read_tokens /
                           1_000_000) * model_costs[model]["cache_read"]
        ...
        table.add_row(
            model, f"{input_tokens:,}", f"{output_tokens:,}",
            f"{cache_write_tokens:,}", f"{cache_read_tokens:,}",
            f"{total_tokens:,}", f"{percentage:.2f}%"
            if model_costs[model]["has_context"] else "Doesn't save context",
            f"${model_cost:.3f}")
...
async def main():
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            console.print(
                Panel("Thank you for chatting. Goodbye!",
                      title_align="left",
                      title="Goodbye",
                      style="bold green"))
            break
        ...
        if user_input.lower() == 'save chat':
            filename = save_chat()
            console.print(
                Panel(f"Chat saved to {filename}",
                      title="Chat Saved",
                      style="bold green"))
            continue
        ...
        if user_input.lower() == 'image':
            ...
            if os.path.isfile(image_path):
                ...
                console.print(
                    Panel("Invalid image path. Please try again.",
                          title="Error",
                          style="bold red"))
                continue
                ...
                try:
                    while automode and iteration_count < max_iterations:
                        try:
                            ...
                            console.print(
                                Panel(f"Error in automode iteration: {str(e)}",
                                      style="bold red"))
                            error_count += 1
                    ...
                    if conversation_history and conversation_history[-1][
                        ...
                        conversation_history.append({
                            ...
                            "assistant",
                            "content":
                            "Automode interrupted. How can I assist you further?"
                        })
            except KeyboardInterrupt:
                console.print(
                    Panel("\nAutomode interrupted by user. Exiting automode.",
            ...
            console.print(
                Panel("Exited automode. Returning to regular chat.",
                      style="green"))
        else:
